...
export const SecureTokenManager = {
  ...
  setLoginTokens: (token, username) => {
    ...
    try {
      localStorage.setItem('github_login_token', encryptedToken);
  ...
  getLoginTokens: () => {
    try {
      const encryptedToken = localStorage.getItem('github_login_token');
      const username = localStorage.getItem('github_login_username');
      const timestamp = localStorage.getItem('github_login_time');
  ...
  hasRepoAccess: () => {
    const { token } = this.getRepoTokens();
    return !!token;
  },
  ...
  validateAllTokens: () => {
    const loginTokens = this.getLoginTokens();
    const repoTokens = this.getRepoTokens();
